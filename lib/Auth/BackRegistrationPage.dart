...
class _RegistrationState extends State<RegistrationPage> {
  ...
  bool checkAuth() {
    ...
    if (name != '') {
      ...
      helper.errorToast('Name filed is required!');
    }
    if (email != '') {
      checkEmail = true;
    } else {
      helper.errorToast('Email filed is required!');
    }

    if (phoneNumber != '') {
      checkPhoneNumber = true;
    } else {
      helper.errorToast('Phone number filed is required!');
    }
    if (password != '') {
      checkPassword = true;
    } else {
      helper.errorToast('Password filed is required!');
    }
    if (confirmPassword != '') {
      checkConPassword = true;
    } else {
      helper.errorToast('Confirm Password filed is required!');
    }
    if (password == confirmPassword) {
      checkPassword = true;
    } else {
      helper.errorToast("Password and Confirm Password dosn't match!");
    }
    if (checkEmail &&
        checkName &&
        checkPhoneNumber &&
      ...
      helper.errorToast('All fields are required');
      return false;
    }
  }
    ...
    if (isLoggedIn) {
      try {
        var response = await http.post(
            Uri.parse('http://192.168.31.135:8000/api/'),
